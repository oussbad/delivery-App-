...
class Validator {
  ...
  Validator required() => _addValidators(
        (value) => GetUtils.isNullOrBlank(value) == true
            ? StringsManager.requiredErrorMessage
            : null,
      );

  Validator isOptional() => _addValidators(
        (value) =>
            GetUtils.isNull(value) || GetUtils.isBlank(value)! ? null : null,
      );

  Validator isUsername() => _addValidators(
        (value) => !GetUtils.isUsername(value!)
            ? StringsManager.usernameErrorMessage
            : null,
      );

  Validator isEmail() => _addValidators(
        (value) =>
            !GetUtils.isEmail(value!) ? StringsManager.emailErrorMessage : null,
      );

  Validator password(int max) => _addValidators(
        (value) => GetUtils.isLengthLessThan(value, max)
            ? StringsManager.passwordErrorMessage
            : null,
      );

  Validator confirmPassword(String password, String confirmPassword) =>
      _addValidators(
  ...
  Validator isPhoneNumber() => _addValidators(
        (value) => !GetUtils.isPhoneNumber(value!)
            ? StringsManager.phoneNumberErrorMessage
            : null,
      );
}
